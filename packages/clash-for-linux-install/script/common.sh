...
ZIP_ARCH_DIR="${ZIP_BASE_DIR}/${ARCH_DIR}"

ZIP_CLASH=$(echo ${ZIP_ARCH_DIR}/clash*)
ZIP_MIHOMO=$(echo ${ZIP_ARCH_DIR}/mihomo*)
ZIP_YQ=$(echo ${ZIP_ARCH_DIR}/yq*)
ZIP_SUBCONVERTER=$(echo ${ZIP_ARCH_DIR}/subconverter*)
...
function _valid_config() {
    [ -e "$1" ] && [ "$(wc -l <"$1")" -gt 1 ] && {
        ...
        echo sudo "$BIN_YQ" '.geodata-mode // false' "$1" >/dev/null
        local is_dat=$(sudo "$BIN_YQ" '.geodata-mode // false' "$1" 2>/dev/null)
        [ "$is_dat" = "true" ] && {
            sudo "$BIN_YQ" -i ".geodata-mode = false" "$1" >/dev/null 2>&1
        }
        msg=$(eval "$cmd") || {
            eval "$cmd"
